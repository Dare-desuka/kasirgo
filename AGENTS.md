...
- `cmd/pos/main.go` — wiring, deteksi LAN.
- `internal/api/handlers.go` — semua endpoint REST + backup (`VACUUM INTO`) + restore.
- `internal/repository/` — `transactions.go` (Checkout atomic, invoiceNumber),
